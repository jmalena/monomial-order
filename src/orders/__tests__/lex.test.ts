...
describe("Lexicographical monomial order (lex) of k-tuples modulo m", () => {
  describe("Ranks", () => {
    /**
     * Enumerate all `k`-tuples modulo `m` in lexicographic order.
     */
    function enumerateLex(k: number, m: number): number[][] {
      return permutationRep(range(m), k).sort((u, v) => lexCompare(u, v));
    }

    /*
     * NOTE: This test is doing exhaustive check for specific parameters `k` and `m`, so be careful with the max value of `k`.
     */
    test.prop([
      fc.integer({ min: 1, max: 4 }),
      fc.integer({ min: 2, max: 16 }),
    ])("rankLex(u; k, m) is bijective on codomain {1, 2, …, m^k}", (k, m) => {
      const maxRank = Number(rankLexMax(k, m));
    ...
    test.prop([
      ...
      fc.integer({ min: 2, max: 16 }),
    ])("unrankLex(rank; k, m) maintains lexicographic order", (k, m) => {
      const rank1 = rankLexRandom(k, m);
      const rank2 = rankLexRandom(k, m);
      const u = unrankLex(k, m, rank1);
      const v = unrankLex(k, m, rank2);

      expect(rank1 <= rank2).toBe(lexCompare(u, v) <= 0);
    });
    ...
    test.prop([
      ...
      (k, m) => {
        ...
        const tuples = ranks.map((rank) => unrankLex(k, m, BigInt(rank)));
    ...
    test.prop([
      ...
      fc.integer({ min: 2, max: 16 }),
    ])("unrankLex(rank; k, m) is left inverse of rankLex(u; k, m)", (k, m) => {
      const rank = rankLexRandom(k, m);
    ...
    test.prop([
      ...
      "unrankLexDeg(rank; d, k, m) is strictly monotone (has lexicographic order)",
      (k, m) => {
        const tuples = enumerateLex(k, m);
        ...
        for (let rank = 2; rank < tuples.length; rank++) {
          ...
          expect(lexCompare(u, v)).toBe(Ordering.LT);
        }
      },
    );
  });
